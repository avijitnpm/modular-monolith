...
<div class="bg-card text-card-foreground rounded-lg border p-4 md:hidden">
	<div class="flex items-center justify-between">
		...
		<span class="inline-block rounded-full px-2 py-0.5 text-xs font-medium {statusColors[user.status] ?? 'bg-gray-100 text-gray-800'}">
			{user.status}
		</span>
	</div>
	<p class="text-muted-foreground mt-1 text-sm">{user.email}</p>
	<div class="mt-2 flex items-center gap-2 text-sm">
		<span class="inline-block rounded-full px-2 py-0.5 text-xs font-medium {roleColors[user.role] ?? 'bg-gray-100 text-gray-800'}">
			{user.role}
		</span>
