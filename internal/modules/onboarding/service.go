...
type IdentityChecker interface {
	HasOrganization(ctx context.Context, zitadelUserID string) (bool, error)
}
...
func (s *Service) CompleteOnboarding(ctx context.Context, zitadelUserID, email, organizationName string) (*OnboardingResult, error) {
	has, err := s.Identity.HasOrganization(ctx, zitadelUserID)
	if err != nil {
		return nil, err
	}
	if has {
	...
	userID, err := s.Users.CreateUser(ctx, createdOrgID, zitadelUserID, email)
	if err != nil {
		return nil, err
	}
